...
Playwright MCP for Operit 将微软官方的 [Playwright MCP](https://github.com/microsoft/playwright-mcp)（v0.0.80）无缝接入 [Operit](https://github.com/AAswordman/Operit)（Android 上的 AI 助手）。基于 Chromium 内核，在手机上获得与桌面浏览器完全一致的渲染与交互能力。
...
**24 个 `browser_*` 工具**覆盖从打开到操作到取证的完整链路，工具面与官方 Playwright MCP v0.0.80 完全一致（官方 stable 版本即 24 个工具）。
...
> ⚠️ raw.githubusercontent.com 在国内可能无法直连，可从 Release zip（v1.0.4）中提取 install.sh。
...
# 2. 准备 Android 源目录（含 mcp.config.json 标志文件 + playwright_mcp.py 转发器）
...
# ⚠️ 新版 Operit 用 venv/bin/python -m playwright_mcp 启动，转发器必须有（缺文件会启动失败）

# 3. 双路径部署（Android 源目录 + Linux 运行目录）
cp -r /sdcard/Download/Operit/mcp_plugins/playwright_mcp /root/mcp_plugins/
```

> 💡 **推荐用方式一一键脚本**：install.sh 会自动生成转发器并双路径部署，避免手动漏步骤。
...
24 个是**正确数量**。官方 @playwright/mcp v0.0.80（当前 npm stable）的 `browser_*` 工具就是 24 个；早期文档（v1.0.0）中的"25"是笔误，**v1.0.1 及以后版本**已修正。

**Q：部署后插件加载不上 / 报 Unknown error？**
大概率是配置里 `pluginMetadata` 字段不完整（缺 `updatedAt` 等会触发 Operit 空指针）。请使用 v1.0.4 的 `install.sh` 或 `config/mcp_config.json`（全字段模板），不要手写精简片段。
